...
class GccAT43 < Formula
  ...
  depends_on "libomp@11.0.0-jos" => :build
  ...
  def install
    ...
    mkdir "build" do
      ...
      ENV.append "CFLAGS", "-I#{Formula["libomp@11.0.0-jos"].opt_include}" if OS.mac?
      ENV.append "LDFLAGS", "-L#{Formula["libomp@11.0.0-jos"].opt_lib}" if OS.mac?
